...
def general_cat_based_question(user_name,question,promp_cat):
    final_response = ""
    asset_found = False
    ...
    for promp_cat in specific_category: 
        if promp_cat=='FAILED':
            ...
            prompt_data_list = []
            for d in data:
                prompt_data_list.append(d.prompt)
            ...
            {prompt_data_list}
            NOTE - If you are not able to find answer then say currently there is no information avaialble on this topic but we will keep you updated.
...
def get_asset_list(token,roles,assets): 
    public_asset_name = {}
    owned_asset_name = {}
    public_asset_list = None
    owned_asset_list = None
    ...
    if "CompanyAdmin" in roles:
        ...
        # validate = token_validation(token)
        response = requests.request("GET", url, headers=headers, data=payload)
        data = response.json()
        print("data at 722 - ",data)
        try:
            page_numbers = data['meta']['last_page_url'].split("=")[-1]
            owned_asset_list = data['data']        
        except:
        ...
        for p in range(2, int(page_numbers)+1):
            ...
            response = requests.request("GET", url, headers=headers, data=payload)
            data = response.json()
            owned_asset_list = owned_asset_list + data['data']

        for names in owned_asset_list:
            name = names['name']
            id = names['id']
            owned_asset_name[id]=name

    # Public assets
    print("Public assets")
    url = "https://api-uat.evident.capital/asset/investor/list?page=1"
    payload = json.dumps({"name":"openai"})
    headers = {
                'Authorization': f'Bearer {token}',
                'Content-Type': 'application/json'
            }
    ...
    for p in range(2, int(page_numbers)+1):
        ...
        response = requests.request("POST", url, headers=headers, data=payload)
        data = response.json()
        public_asset_list = public_asset_list + data['data']

    for names in public_asset_list:
        name = names['name']
        id = names['id']
        public_asset_name[id]=name
    print("public_asset_name - ",public_asset_name)
    return public_asset_list, owned_asset_list, public_asset_name,owned_asset_name 

# get_asset_list("MzU3Ng.v9qqM_e_86P9UlT7025t9O1fX91ioEo04KxI0WI0iuahVL4ImiJX1uV7Jr2N",['CompanyAdmin'])
...
def get_asset_based_response(user_name,assets_identified,asset_list,question):
    # Convert assets_identified to a set for faster lookups
    assets_identified_set = set(map(int, assets_identified))

    # Use a single loop to filter assets
    assets = [ass for ass in asset_list if int(ass['id']) in assets_identified_set]
    prompt = f"""Customer:{user_name} is not providing you any information, all information is with you, DO NOT SAY TO CUSTOMER THAT THEY HAVE NOT PROVIDED INFORMATION. 
        You are smart and intelligent chat-bot having good knowledge considering this chat with user. Provide answer in a way that you are chatting with customer. Do not use any kind of emojis. Do not greet user while answering. 
        To get proper trade values, add all results of that perticular assets. 
        e.g. if you want overall records of units then it will be sum of all units of respective column, to get final unit counts add all values of units.
        NOTE - Keep tone positive and polite while answering user's query.
    ...
    response = get_gemini_response(question,prompt)
    response = get_gemini_response(response,"""Ensure the response keeps the provided information intact without altering or modifying any details.
                                                    If certain information is unavailable, state politely, currently We're unable to provide this specific details at the moment, but we're here to assist you further if needed.
                                                    Else, Keep information as it is.
                                                    Avoid mentioning or implying that the user has not provided information or response in requested format.
                                                    Do not greet the user in your response.
                                                    Use proper formatting such as *, line breaks, and other formatting styles to enhance readability while keeping answer as it is.
...
def handle_questions(token, roles, user_name, questions,chat_session_id, convos):     
    asset_found = False
    response = ''
    ...
    if 'Greetings' in promp_cat[0] and len(promp_cat)==1:
        response = f"Hey {user_name[0]+user_name[1:].lower()}! How can I help you?"
    ...
    if 'Personal_Assets' in promp_cat:        
        public_asset_list, owned_asset_list, public_asset_name,owned_asset_name = get_asset_list(token,roles)

        prompt = f"""1. Public assets - {public_asset_name}
        2. Owned assets - {owned_asset_name}        
        From above asset details(where key is asset id and value is asset name) identify if question is about any specific asset, owned asset, or all assets.
        If question is about specific assets then STRICTLY ONLY return key, if there is more than one asset then separate them with coma(,). E.g. 44,27
        else return Owned_Asset if asset is present in Owned assets list else Public_Assets""" 
        asset_response = get_gemini_response("".join(questions),prompt)
        print("asset_response - ",asset_response)
        if asset_response in "Owned_Asset":
            asset_list = owned_asset_list
            asset_names = owned_asset_name
        elif asset_response in "Public_Assets":
            asset_list = public_asset_list
            asset_names = public_asset_name
        else:
            asset_list = public_asset_list + owned_asset_list
            asset_names = public_asset_name.update(owned_asset_name)
        
        prompt = f"""This is dictionary of assets where key represents id of asset and value represents name of asset.
        Asset Dictionary - {asset_names}
        Based on this identify question is about which asset. If no specific asset then STRICTLY return 0 else return STRICTLY only those ids which are present in question.
        If there is more than one asset then separate them with coma(,). E.g. 44,27
        Examples:- 
        Question - Give me list of all my assets or how many assets I have?
        Answer - 1,2,3,4,5,6
        Question - tell me about asset1
        Answer - 1
        Question - tell me about asset1 and asset2
        Answer - 1,2
        """
        asset_response = get_gemini_response(question,prompt)
        print("response 913 - ",asset_response)
        assets_identified = asset_response.strip().split(",")
        print("assets_identified - ",assets_identified)
        if len(assets_identified)>0 and int(assets_identified[0])!=0:
            asset_found=True
            response = get_asset_based_response(user_name,assets_identified,asset_list,question)
            return response,asset_found
        ...
        if previous_question_status==False:
            ...
            response, asset_found = general_cat_based_question(user_name,question,promp_cat)                
        else:
            ...
            if int(response.strip())==1:
                prompt = f"""Identify asset name about which user is asking from following assets dictionary - {asset_names}
                        STRICTLY ONLY return key, if there is more than one asset then separate them with coma(,). E.g. 44,27
                        If no asset found then return only 0, If its normal greeting message then also return 0.
                        If there are more than one asset with similar name then take only most relevant asset id.""" 
                asset_response = get_gemini_response("".join(questions),prompt)
                print("asset_response - ",asset_response)
                
                assets_identified = asset_response.split(",")
                if len(assets_identified)>0 and int(assets_identified[0])!=0:
                    asset_found=True
                    response = get_asset_based_response(assets_identified,asset_list,question)
                    return response,asset_found
                else:
                    response, asset_found = general_cat_based_question(question,promp_cat) 
            else:
...
# Identify question
# question = "is there any highlight available for spacex?"
# promp_cat = get_prompt_category(question)
# promp_cat = promp_cat.split(",")
# print(promp_cat)
# prompt = """Check if question belongs to below mentioned category or not.
#             Personal_Assets: Following details are present for variety of assets like openai, spacex and many more - These assets include various categories such as Private Equity, Venture Capital, 
#             Private Credit, Infrastructure, Hedge Funds, Digital Assets, Real Estate, Collectibles, 
#             Structuring, Private Company Debenture, Note, Bond, Fund, and Equity. 
#             These assets have a range of Internal Rate of Return i.e. IRR. 
...
#             that asset, commitments. Here user asks about different things on asset. 
#             These are user specific assets. When user wants to know about his/her asset details.
#             return category name 'Personal_Asset' if question belongs to this category else return 'Others'"""
# response = get_gemini_response(question,prompt)
# print(response)
...
def login(request):
    ...
    payload = json.dumps({
    "email": "sai.mansanpally+2001ind@evident.capital",
    ...
    print("token - ",token)
    print("2FA Response - ", headers, response)
    data = response.content
    print(data)
    user_url = "https://api-uat.evident.capital/user/me"
    headers = {
                'Authorization': f'Bearer {token}',
                'Content-Type': 'application/json'
            }
    ...
    response = create_chat_session(user_id)
    data = json.loads(response.content)
    chat_session_id = data['data']['id']   
    return JsonResponse({"token":token,'chat_session_id':chat_session_id},status=200)
...
def evidAI_chat(request):
    try:
        ...
        if request.method == 'POST':
            ...
            token_valid,user_id,user_name,roles = token_validation(token)
            if token_valid is None:
                # print("valid token none")
                log_message("error",f"Invalid Token, Token: {token}")            
                return JsonResponse({"message":"Invalid user, please login again","data":{"response":"Failed to validate token for user, please check token"},"status":False},status=200)
            ...
            response, asset_found = handle_questions(token, roles, user_name, questions, chat_session_id, conversation_history)
