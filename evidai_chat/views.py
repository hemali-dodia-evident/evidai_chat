...
def handle_questions(token, roles, user_name, questions,onboarding_step):     
    ...
    if FAILED and len(questions)>1:
        prompt = f"""Identify if current question is related or is in context of previous questions or not And related to assets or not. If related return 1 else 0. previous questions - {questions}"""
        response = get_gemini_response(question,prompt)
        try:
